...
def is_bst(t):
    ...
    def bst_min(tree):  # 默认已经是valid
        if tree.is_leaf():
            return tree.label
        elif len(tree.branches) == 1:
            if tree.branches[0].label > tree.label:
                return tree.label
            else:
    ...
    if t.is_leaf():
        ...
        return is_bst(t.branches[0]) and is_bst(t.branches[1]) and (bst_max(
            t.branches[0]) <= t.label < bst_min(t.branches[1]))
...
def path_yielder(t, value):
    ...
    for tree in t.branches:
        for path in path_yielder(tree, value):
            # use for loop to iterate the iterator generated by the generaror
            yield [t.label] + path
